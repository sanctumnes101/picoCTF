...
Let's connect to the server in the `Description`.
...
The hints give us nothing useful to start with. Let's try the title! I googled "la cifra de cipher", and i got [this](https://en.wikipedia.org/wiki/Vigen%C3%A8re_cipher) wikipedia page. Apparently the message is encryted using `Vigenère cipher`, which is based on the caesar encryption. You can read more about it in the wikipedia article.

Let's decode the message using [this site](https://www.guballa.de/vigenere-solver)
...
As you can see, the flag is: `picoCTF{b311a50_0r_v1gn3r3_c1ph3r6fe60eaa}`
